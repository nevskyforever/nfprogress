...
`Build Tauri All.sh` открывает отдельные окна Terminal для ARM- и Intel-сборки,
поэтому они выполняются параллельно. `Release Tauri All.sh` аналогично запускает
оба релиза; обновление общего манифеста при этом выполняется по очереди, чтобы
не потерять запись одной из архитектур.
