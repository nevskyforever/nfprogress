...
class WritingProject {
    ...
    private func cumulativeProjectProgress(upto entry: Entry, includeEntry: Bool) -> Int {
        var total = 0
        // Начальный прогресс для каждого этапа определяется по факту появления
        // первой записи этого этапа. Это позволяет корректно учитывать ситуации,
        // когда этап был создан заранее, но записи в нём появились позже.
        var stageStart: [UUID: Int] = [:]
        ...
        for e in sortedEntries {
            ...
            if e.syncSource == .scrivener {
                if let stage = stageForEntry(e) {
                    if stageStart[stage.id] == nil {
                        stageStart[stage.id] = total
                    }
                    total = (stageStart[stage.id] ?? 0) + e.characterCount
                } else {
