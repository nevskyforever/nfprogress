...
<li>Завершённый этап отмечается в общей карте значком ✅. Его существующая карта и ветвь в общей карте доступны только для просмотра. Если карта этапа не создавалась, кнопка «Карта» показывает сообщение, а в общей карте выводится пояснение вместо пустой ветви. Карта завершённого проекта также доступна только для просмотра.</li>
...
<li>A completed stage is marked with ✅ in the combined map. Its existing map and its branch in the combined map are read-only. If no stage map was created, the “Map” button shows a message and the combined map displays an explanation instead of an empty branch. A completed project map is also read-only.</li>
...
<li>Una etapa finalizada se marca con ✅ en el mapa combinado. Su mapa existente y su rama en el mapa combinado son de solo lectura. Si no se creó un mapa para la etapa, el botón «Mapa» muestra un mensaje y el mapa combinado presenta una explicación en lugar de una rama vacía. El mapa de un proyecto finalizado también es de solo lectura.</li>
...
<li>Eine abgeschlossene Etappe wird in der Gesamtkarte mit ✅ gekennzeichnet. Ihre vorhandene Karte und ihr Zweig in der Gesamtkarte sind schreibgeschützt. Wenn keine Etappenkarte erstellt wurde, zeigt die Schaltfläche „Karte“ eine Meldung an und die Gesamtkarte enthält anstelle eines leeren Zweigs einen Hinweis. Die Karte eines abgeschlossenen Projekts ist ebenfalls schreibgeschützt.</li>
...
<li>Une étape terminée est signalée par ✅ dans la carte globale. Sa carte existante et sa branche dans la carte globale sont en lecture seule. Si aucune carte n’a été créée pour l’étape, le bouton « Carte » affiche un message et la carte globale présente une explication au lieu d’une branche vide. La carte d’un projet terminé est également en lecture seule.</li>
...
<li>Uma etapa concluída é marcada com ✅ no mapa combinado. O mapa existente e a ramificação dessa etapa no mapa combinado são somente leitura. Se nenhum mapa da etapa tiver sido criado, o botão “Mapa” exibirá uma mensagem e o mapa combinado mostrará uma explicação em vez de uma ramificação vazia. O mapa de um projeto concluído também é somente leitura.</li>
