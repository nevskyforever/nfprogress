...
def refresh_test_data_profile(
    ...
    """Build and atomically activate test data from its authoritative source.

    If any legacy source is present, the production SQLite mirror is excluded
    from the snapshot regardless of its mtime or health marker.  This is the
    bridge contract: PKL/JSON wins until the real cutover.
    """
    ...
    snapshot = _authoritative_test_snapshot(source)
    staging_profile: Path | None = Path(tempfile.mkdtemp(
        prefix=f'.{destination.name}-refresh-', dir=destination.parent,
    ))
    ...
    try:
        ...
        for name in _known_files(snapshot):
            if name != 'nfprogress.db':
                shutil.copy2(snapshot / name, staging_profile / name)
        report = prepare(snapshot, staging_profile, replace=True)
        ...
        if destination.exists():
            rollback = destination.parent / (
                f'.{destination.name}-refresh-rollback-'
                f'{datetime.now(timezone.utc).strftime("%Y%m%dT%H%M%S.%fZ")}'
            )
            os.replace(destination, rollback)
        backup_relative = (
            Path(report.backup).relative_to(staging_profile)
            if report.backup else None
        )
        try:
            os.replace(staging_profile, destination)
        except Exception:
            if rollback is not None and rollback.exists():
                os.replace(rollback, destination)
            raise
        staging_profile = None
        report.bundle = str(destination / 'nfprogress-migration.json')
        if backup_relative is not None:
            report.backup = str(destination / backup_relative)
        report.details.extend([
            'source_authority=legacy' if any(
                (snapshot / name).is_file()
                for name in ('data.pkl', 'settings.pkl', 'gamer.pkl', 'documents.json')
            ) else 'source_authority=sqlite',
            'profile_activation=atomic',
            *([f'profile_rollback={rollback}'] if rollback else []),
        ])
        return report
    finally:
...
def _authoritative_test_snapshot(root: Path) -> Path:
    """Snapshot bridge sources without copying a stale/dirty source mirror."""
    directory = Path(tempfile.mkdtemp(prefix='nfprogress-test-source-'))
    legacy_names = ('data.pkl', 'settings.pkl', 'gamer.pkl', 'documents.json')
    try:
        present = [name for name in legacy_names if (root / name).is_file()]
        names = present or _known_files(root)
        if present and (root / 'external_file_manifest.json').is_file():
            names.append('external_file_manifest.json')
        for name in names:
