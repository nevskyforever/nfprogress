...
| SQLite shadow mirror | [done] | `settings` and `notes` are SQLite-authoritative. Startup performs controlled, parity-verified imports from PKL and switches ownership only inside successful SQLite transactions. Python uses typed repositories; Tauri exposes fixed parameterized Notes/settings commands with ownership guards. `projects/stages/progress` and `game` remain PKL-authoritative and continue to mirror into SQLite |
...
| Statistics frontend | [in progress] | `frontend/src/core/statistics` owns parity-tested pure calculations and the adapter supplies them to the existing UI; SQLite project DTOs now provide read entries for future/local pure-statistics integration, while Python `/statistics` remains the reference/source for freezes and streak state |
| Notes/Mind Elixir frontend | [done] | Legacy pastel full-card note colors and in-card colour palette, a prominent full-width create action, autosave, and canonical Mind Elixir assets use a storage-neutral NotesRepository. Tauri CRUD/order uses typed Rust/SQLite commands; map/XMind operations remain API-backed while preserving SQLite note identity and `#карта` synchronization |
...
| Settings frontend | [done] | Existing UI is unchanged. Web/non-Tauri uses FastAPI; Tauri uses the typed SQLite repository for ordinary settings reads/writes, while project-coupled transitions retain their existing transactional API path |
