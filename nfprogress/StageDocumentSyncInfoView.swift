...
struct StageDocumentSyncInfoView: View {
    ...
    private var info: String {
        ...
        case .word:
            let url = DocumentSyncManager.resolveURL(bookmark: &stage.wordFileBookmark,
                                                     path: stage.wordFilePath)
            let name = url?.lastPathComponent ?? (stage.wordFilePath.flatMap { URL(fileURLWithPath: $0).lastPathComponent }) ?? ""
    ...
    private var syncURL: URL? {
        switch stage.syncType {
        case .word:
            return DocumentSyncManager.resolveURL(bookmark: &stage.wordFileBookmark,
                                                  path: stage.wordFilePath)
        case .scrivener:
            return DocumentSyncManager.resolveURL(bookmark: &stage.scrivenerProjectBookmark,
                                                  path: stage.scrivenerProjectPath)
        case .none:
            return nil
        }
