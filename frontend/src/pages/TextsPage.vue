...
function stageNameFor(document: ProjectDocument): string {
  const project = projectFor(document)
  return document.stage_id
    ? project?.stages.find((stage) => stage.id === document.stage_id)?.name ?? t('Удалённый источник')
    : ''
}
