...
function harness() {
  ...
  const runtime = new NoteSyncRuntime({
    auth: userAuth, identityRepository: identity, bindings: bindings as never, keys: keys as never, orchestrator: orchestrator as never,
  })
  return { runtime, userAuth, identity, bindings, keys, orchestrator, activate: () => { active = true } }
}
...
describe('headless normal-user Notes sync runtime', () => {
  ...
  it('unlocks the existing key context and runs exactly one bounded cycle', async () => {
    const h = harness(); await h.runtime.login('user', 'account-password')
    await expect(h.runtime.unlock('e2ee-passphrase', { maxPullPages: 1 })).resolves.toMatchObject({ identity: IDENTITY, sync: EMPTY_RESULT })
    expect(h.keys.unlockWithPassphrase).toHaveBeenCalledWith(IDENTITY.local_account_id, 'e2ee-passphrase')
    expect(h.orchestrator.runOnce).toHaveBeenCalledWith(IDENTITY.local_account_id, IDENTITY.device_id, { maxPullPages: 1 })
  })
  ...
  it('joins concurrent explicit retries for one authenticated key context', async () => {
    ...
    h.orchestrator.runOnce.mockClear()
    const pending = deferred<typeof EMPTY_RESULT>()
    h.orchestrator.runOnce.mockReturnValueOnce(pending.promise)
    const first = h.runtime.retry(); const second = h.runtime.retry()
    await vi.waitFor(() => expect(h.orchestrator.runOnce).toHaveBeenCalledTimes(1))
