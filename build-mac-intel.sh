#!/bin/bash
BUILD_LOCK_DIR="${TMPDIR:-/tmp}/nfprogress-intel-build-${UID}.lock"
if ! mkdir "$BUILD_LOCK_DIR" 2>/dev/null; then
  echo "Ошибка: Intel-сборка уже выполняется. Дождитесь её завершения."
  exit 1
fi
trap 'rmdir "$BUILD_LOCK_DIR" 2>/dev/null || true' EXIT
if pgrep -f '[n]uitka.*--output-dir=build-intel' >/dev/null; then
  echo "Ошибка: Intel-сборка уже выполняется. Дождитесь её завершения."
  exit 1
fi
