...
The Web panel is directly reachable at `/admin/login` and `/admin`, outside ordinary AppShell/workspace bootstrap and absent from desktop/mobile navigation. It retains access and refresh tokens only in runtime memory; reload requires login. A protected request that receives `401 invalid_token` serializes one refresh request, rotates both in-memory values, and retries the original request once. A failed or malformed refresh clears the session and returns the admin surface to login. Login also clears/revokes its just-created session if `/account/me` fails or proves that the account is not an active admin. Tokens are never written to browser storage, cookies, IndexedDB, or URLs.

The Users view has stable server pagination (50 rows, Previous/Next), resets offsets on filters, and moves back to an available page after a result set shrinks. Its labels use the existing Vue locale store; backend canonical status and role values, usernames, and emails remain data rather than translated authority.
