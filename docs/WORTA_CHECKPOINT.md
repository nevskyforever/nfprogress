...
**Текущий этап:** **C16 Desktop Sync** (вес 3,0) — `IN PROGRESS`, `FINAL GUI ACCEPTANCE PASSED / LOCAL COMMIT READY / PUSH AND INDEPENDENT REMOTE CI PENDING`.\
**Следующий зачёт:** после полного закрытия C16 — **73,0%**.\
**Последний подтверждённый remote HEAD:** `6c7edbfa24289fcfca39fc754eaff9a28e419dbc`.\
**Последняя remote-проверка:** по подтверждению пользователя оба C16 workflow на опубликованном correction прошли; Windows Rust и frontend admin также зелёные. C16 не закрыт из-за последующего GUI blocker и незавершённой live safety acceptance.
**Текущая задача Codex:** C15 официально закрыт после remote acceptance. C16 Pass 1/2/3B/3C опубликованы, оба remote CI workflow прошли; основной A→B live sync/import и двустороннее редактирование подтверждены. Final GUI manual acceptance всех пяти C16 Notes/navigation/sync сценариев прошла; накопленные GUI corrections готовы к публикации. C16 не закрыт до пользовательского push и independent remote CI. Самостоятельно C17 или C18 не начинать. Legacy document interval не использовать; C17 сохраняет conflict resolution.
...
**Независимая C17:** в отдельном worktree/ветке `6.0-C17` Pass 1 и Pass 2A опубликованы и прошли CI; последний опубликованный SHA — `698836f5502b48a9be57c31d42be519332c097c7`. Этот worktree в C16 не менялся. Порядок дальше: закрыть C16, перенести его исправления в C17, завершить C17 и выполнить PR с base `6.0`, compare `6.0-C17`. Утверждённый Web First roadmap, C18 compression-before-E2EE и encrypted project metadata, C22 storage quotas и GNU GPLv3 dependency gate сохраняются. C16 остаётся `IN PROGRESS`; официальный прогресс WORTA — **70,0%**.

## 69. C16 final GUI manual acceptance (локальный commit готов, remote gate ожидается)

Пользователь завершил final manual acceptance C16: **все пять проверок PASS** — (1) многократное открытие/закрытие editor; (2) editor после Settings → Notes; (3) переходы между Settings, Notes и Projects; (4) редактирование и сохранение Note после sync; (5) повторный sync, его индикатор и доступность интерфейса после завершения. Накопленные C16 GUI corrections (HTML hydration, Ionic modal lifecycle, scoped Notes outlet remount, navigation cleanup и сброс `cloudSession.busy` после 401) готовы к единому локальному commit и публикации. C16 по-прежнему **не `CLOSED`**: официальный прогресс остаётся **70,0%** до push и независимой проверки GitHub Actions.

`6.0-C17` остаётся отдельной веткой/worktree: опубликованный SHA `698836f5502b48a9be57c31d42be519332c097c7`, Pass 1 и Pass 2A прошли remote CI; **Pass 2B ещё не начинался**. После закрытия C16 его corrections переносятся из `6.0` в `6.0-C17`; после завершения C17 пользователь создаёт первый Pull Request с base `6.0` и compare `6.0-C17`. Web First roadmap, C18 compression-before-E2EE и encrypted project metadata, C22 storage quotas и GPLv3 dependency gate сохраняются без изменений.
