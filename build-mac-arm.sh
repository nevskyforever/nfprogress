...
for QT_LANGUAGE in "${QT_TRANSLATION_LANGUAGES[@]}"; do
  if ! find build-arm -type f -name "qtbase_${QT_LANGUAGE}.qm" -print -quit | grep -q .; then
    echo "❌ Перевод qtbase_${QT_LANGUAGE}.qm не попал в ARM-сборку!"
    exit 1
  fi
done
...
if ! find build-arm -type f -path "*/notes_assets/vendor/muuri.min.js" -print -quit | grep -q .; then
  echo "❌ Редактор заметок не попал в ARM-сборку!"
  exit 1
fi
