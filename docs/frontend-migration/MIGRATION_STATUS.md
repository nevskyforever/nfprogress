...
| SQLite shadow mirror | [in progress] | `projects`, `notes`, and `game` remain pickle-owned; `settings` is SQLite-authoritative. Startup imports and verifies `settings.pkl` before the transactional ownership switch. Python uses the SQLite settings repository after cutover, and Tauri exposes fixed typed settings read/write commands with an ownership guard. `settings.pkl` remains legacy/non-authoritative; no other subsystem was cut over |
